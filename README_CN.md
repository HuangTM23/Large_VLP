...
- **窗口损失 (Window Loss)**：V2 模型现在采用重叠轨迹窗口的 MSE 损失，而不再是单点对比。这增强了局部轨迹的平滑性约束和形状一致性。
- **两种训练模式**：
...
| 特性 | V2 (基线模型) | MultiHead (高级模型) | Hierarchical (分层模型) | V3 (拓扑版) |
| :--- | :--- | :--- | :--- | :--- |
| **注意力** | 单头全局注意力 | **三头注意力** | **空间感知 (块级)** | **隐式 (LSTM 引导)** |
| **信号处理** | **窗口输入 + 时间学习** | 分层处理 (强/弱) | **CNN + 空间注意力** | **1D-CNN 指纹 + 傅里叶** |
| **整合器** | LSTM | LSTM | **全局 LSTM** | **LSTM (状态初始化)** |
| **最佳场景** | 稳定环境 | 复杂、动态环境 | **超长轨迹 (防OOM)** | **高精度与强鲁棒性** |
| **代码位置** | `..._v2.py` | `..._multihead.py` | `..._hierarchical.py` | `..._v3.py` |
...
### 1. 训练基线模型 (V2)
每个时间步现在输入一个 RSS 窗口。模型自动学习窗口内的时间权重，相比简单的均值滤波具有更强的鲁棒性。
```bash
python3 train.py --model v2 --epochs 3000
```

### 2. 训练多头模型 (MultiHead)
采用 **基于不确定性的自动加权 (Kendall)** 来平衡位置 MSE 和航向损失 (Heading Loss)。
```bash
python3 train.py --model multihead --epochs 1000
```

### 3. 训练层次化模型 (Hierarchical)
采用 **CNN + 空间感知注意力** 混合子网络。实现了块级连续性约束，确保超长路径下的轨迹连贯性。
```bash
...
### 4. 训练拓扑感知模型 (V3)
目前最先进的模型版本，包含以下创新：
- **隐式记忆注意力 (Implicit Memory)**：利用 LSTM 隐状态指导注意力计算，不再依赖不稳定的坐标反馈，有效抑制误差累积。
- **拓扑 LED 编码器**：结合 **傅里叶特征映射 (Fourier)** 与 **图卷积 (GCN)**，对 LED 的物理邻居关系进行深度建模。
- **RSS 指纹提取**：使用 1D-CNN 提取 12 通道间的比例关系（拓扑指纹），增强了对环境增益波动的鲁棒性。
- **三任务自动平衡**：自动调节 **位置 MSE**、**航向损失 (Heading)** 和 **几何一致性损失 (Topology)** 的权重。
```bash
